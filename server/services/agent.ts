...
function buildTools(customerId: number) {
  ...
  return {
    searchProducts: tool({
      description: 'Search the product catalog by name or description. Returns up to 5 matching products.',
      inputSchema: z.object({
        query: z.string().describe('Search terms, e.g. "earbuds" or "charger"'),
      }),
      execute: async ({ query }) => {
